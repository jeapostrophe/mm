...
(provide
 (contract-out
  [make-heap-gui
   (-> boolean? symbol? vector?
       (-> void?))]))

(define (make-heap-gui step? FREE v)
  (define hv (new heap-viz% [heap-vec v] [step? step?] [FREE FREE]))
  (λ ()
    (send hv update-view)))
